<template lang="pug">
  ...
  .grid-menu(ref="container" @mouseleave="onBtnLeave")
    .header(ref="header").clearfix

      .clear-all(@mouseenter="onBtnLeave"): button.btn.btn-sm.btn-outline-dark(type="button" @click="onClearAll" :disabled="btnActive.row === null && btnActive.col === null" :class="{active: !(btnActive.row === null && btnActive.col === null)}"): fa(icon='broom')
    ...
    .rows-buttons
      ...
      .buttons(:style="{gridTemplateRows: ' 1fr'.repeat(data.rows.length), gridTemplateColumns: ' 1fr'.repeat(data.cols.length)}")
        ...
        lines.push(
          {
            ...
            t: parseInt($col.css('margin-top')) + $col.position().top + $col.outerHeight(),
            l: paddingLeft + ((width - this.headerColGapLines * (cols.length - 1)) / cols.length) * (i + 0.5) + this.headerColGapLines * i,
            h: outerHeight - (parseInt($col.css('margin-top')) + $col.position().top + $col.outerHeight())
          }
        );
      });
